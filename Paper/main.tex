...
Die wichtigste Designentscheidung und damit Grundidee des Layouts ist es, jede Gruppe in einem Kreis zu kapseln. Sie wird also innerhalb eines Kreises dargestellt und Kanten über Gruppengrenzen werden nur über feste Ports zugeführt. Eine Kante von einem Knoten in einer Gruppe geht also zunächst zu einem Port der Gruppe und dann zu einem Knoten außerhalb der Gruppe.\todo[inline]{Port Definition ok so?}
...
\todo[inline]{Mehr Details. Welche Kräfte und zwischem wem? dazu Anker}
\todo[inline]{Mehr Verweise, Andreas hat noch Eader et al und Fruchterman + Reingold erwähnt}

%\myparagraph{Anker}
\todo[inline]{Abschnitt bei Kräfte einbauen}
...
\SetAlgoLined
\Ein{Graph $G=(V,E)$ mit Gruppenbaum $S$ } % mit Koten $V$ mit Größe, gerichteten \& gefärbten Kanten $E$, Mengen $S$ von Knoten und Elementen aus $S$
\Aus{Gruppen-hierarchisches Layout von $G$ } % mit diesen und jenen Eigenschaften, evtl Name einführen für Referenzierung
$d = $ Länge des längsten zusammenhängenden Pfads in $S$\; %niedrigste Stufe einer Gruppe 
$i = d$\;
\Solange{$i > 0$}{ %i > 0 da Dummy-Wurzelknoten des Gruppenbaums ausgeschlossen werden muss % benötigt, dass oberste Ebene auch Gruppe
...
$i = 1$\; %1 sollte die Ebene unter der Dummy Wurzel sein
\Solange{$i \leq$ d}{ %Anzahl Stufen % benötigt, dass oberste Ebene auch Gruppe
  \Fuer{Jede Gruppe $H$ auf Stufe $i$} {
...
\end{algorithm}
\todo[inline]{Algo ist korrigiert muss aber korrekturgelesen werden}
\todo[inline]{Algo-Zeilennummern in Texten an Änderungen anpassen}
...
\todo[inline]{Ignaz hatte noch "nächste-niedrigen Stufe" angestrichen, hoffe das Substitut ist ok}
	% top-down
...
\todo[inline]{ok ohne Unterüberschrift?}
...
\todo[inline]{Einbauen von Ports hier so ok?}
...
%, d.h. wir haben die Winkel an denen Gruppengrenze-überschreitenden Kanten den Kreis schneiden sollen. 
...
\todo[inline]{Überschrift besser als ``.. beim Öffnen oder Schließen einer Gruppe''?}
% was betrachten?
